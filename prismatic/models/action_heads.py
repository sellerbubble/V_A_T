...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.model = MLPResNet(
            num_blocks=2, input_dim=input_dim*action_dim, hidden_dim=hidden_dim, output_dim=ACTION_DIM_OUTPUT
        ) # here we set output dim to 7, since we are predicting 7 actions

    def predict_action(self, actions_hidden_states, action_chunk=NUM_ACTIONS_CHUNK):
        # actions_hidden_states: last hidden states of Transformer corresponding to action tokens in sequence
        # - shape: (batch_size, chunk_len * action_dim, hidden_dim)
        # ground_truth_actions: ground-truth actions
        # - shape: (batch_size, chunk_len, action_dim)
        batch_size = actions_hidden_states.shape[0]
        rearranged_actions_hidden_states = actions_hidden_states.reshape(batch_size, action_chunk, -1)
...
class DiffusionActionHead(nn.Module):
    ...
    def __init__(
        ...
        action_chunk=NUM_ACTIONS_CHUNK,
    ):
        super().__init__()
