...
class SeparatedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        if num_non_act == 256 or num_non_act == 261:
            ...
            qkv_act = self.qkv_non_act(act_tokens)
            q_act = qkv_act.reshape(B, self.vat_tokens, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)[0]
            
            attn_act = (q_act @ k_non.transpose(-2, -1)) * self.scale
            attn_act = attn_act.softmax(dim=-1)
            attn_act = self.attn_drop(attn_act)
            x_act = (attn_act @ v_non).transpose(1, 2).reshape(B, self.vat_tokens, C)
            
            x_non = self.proj_non_act(x_non)
            x_non = self.proj_drop_non_act(x_non)
            x_act = self.proj_non_act(x_act)
            x_act = self.proj_drop_non_act(x_act)
            ...
            attn_act = (q_act @ k.transpose(-2, -1)) * self.scale
            attn_act = attn_act.softmax(dim=-1)
            attn_act = self.attn_drop(attn_act)
            x_act = (attn_act @ v).transpose(1, 2).reshape(B, self.vat_tokens, C)
...
class SeparatedAttentionBlock(nn.Module):
    ...
    def forward(self, x, task_embedding):
        ...
        norm_act = self.norm1_non_act(act_tokens)
        ...
        norm_act2 = self.norm2_non_act(act_tokens)
        norm_non_act2 = self.norm2_non_act(non_act_tokens)
        
        act_out = self.mlp_non_act(norm_act2)
...
class VATVIT(VisionTransformer):
